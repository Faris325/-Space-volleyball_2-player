...
func _physics_process(delta): # глобальный цикл, все что находится в нем,исполняется постоянно.
	...
	if Input.is_action_just_pressed("ui_accept") and is_on_floor():  # прыжок
	...
	var direction = Input.get_axis("left", "right") # Направление: перемещение  (получает значение 1 0 -1) 
	if direction:
